...
| Phase | Scope | Status |
|---|---|---|
| 1 | Serialize `DatabaseOperationHelper` token state (lock around cache + token-file I/O), fix the `.zone`/`.server` key bug, make `NotificationTokenHelper` `Sendable` | Planned |
